...
class Runner():
    ...
    def _forward_step(self, batch_x, frame_y, lengths, phase='train', drop_last=True):
        frame_p, dwsmp_p = self.model(batch_x, lengths)
    
        mask = (torch.arange(max(lengths))[None, :] < lengths[:, None]).to(batch_x.device)
        dwsmp_loss = self.criterion['dwsmp'](dwsmp_p, frame_y, mask)
        isolt_loss = self.criterion['isolt'](frame_p)
        ...
        loss = frame_loss + dwsmp_loss + 0.1 * isolt_loss
        ...
        self.metrics[f'{phase}/dwsmp_loss'] += dwsmp_loss.item() / reduce
        self.metrics[f'{phase}/isolt_loss'] += isolt_loss.item() / reduce
        ...
        while self.epoch < self.max_epoch:
            ...
            for phase in ['train', 'valid']:
                ...
                for _, batch_x, frame_y, utter_y, lengths in tqdm(self.loader[phase], ncols=100):
                    ...
                    if phase == 'train':
                        frame_p = self._forward_step(batch_x, frame_y, lengths, phase)
                    else:
                        with torch.no_grad():
                            frame_p = self._forward_step(batch_x, frame_y, lengths, phase, drop_last=False)
                    ...
                    for true, pred, l in zip(frame_y, frame_p, lengths):
                        ...
                        utter_pred.append(0 if cnt >= 3 else 1)

                    utter_true += list(utter_y.numpy())
    ...
    def test(self, checkpoint):
        ...
        for _, batch_x, frame_y, utter_y, lengths in tqdm(self.loader['test'], ncols=100):
            ...
            frame_p = self._forward_step(batch_x, frame_y, lengths, phase='test')
